...
def download_RM(outputdir):
		...
		if 'abstract' in RM['RM'][RM_class].keys():
			...
			for j in range(0,len(classList)):
				...
				if 'inherit' in RM['RM'][next_RM_class].keys():
					inherit_list = RM['RM'][next_RM_class]['inherit'].split(', ')
					if RM_class in inherit_list:
						...
						if RM_class == 'DATA_VALUE' and 'DV_CODED_TEXT' not in RM['RM'][RM_class]['specialization']:
							RM['RM'][RM_class]['specialization'].append('DV_CODED_TEXT')
							RM['RM'][RM_class]['specialization'].append('DV_EHR_URI')
	...
	for url in URL_list:
		...
		for r in results:
			...
			if class_url[0] == '#' and ( item[-5:]=="Class" or item[-9:]=="Interface" or item[-11:]=="Enumeration" ) :
				raw = item.strip()
				clean = re.sub(r'^[\d.]+\s*', '', raw)   # remove "5.4.6. "
				clean = re.sub(r'\s*Class$', '', clean)   # remove " Class"
				clean = re.sub(r'\s*Interface$', '', clean)
				clean = re.sub(r'\s*Enumeration$', '', clean)
				item = clean
				df_out.loc[len(df_out)] = [url+class_url, item]
